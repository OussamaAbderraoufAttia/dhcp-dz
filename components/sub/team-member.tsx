...
interface TeamMemberProps {
  ...
  role: string;
  image: string;
  linkedin: string;  // LinkedIn URL as a prop
  cv: string;        // CV URL as a link (could be a webpage or any URL)
}
      ...
      <div className="relative h-64 w-full">
        ...
        <div className="absolute inset-0 bg-blue-600 bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-300 flex items-center justify-center">
          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex space-x-3">
            {/* LinkedIn */}
            <Link href={linkedin} className="bg-white p-2 rounded-full text-blue-600 hover:bg-blue-50 transition-colors">
            ...
            {/* CV Link */}
            <Link href={cv} className="bg-white p-2 rounded-full text-blue-600 hover:bg-blue-50 transition-colors" target="_blank" rel="noopener noreferrer">
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
